...
(comment
  ;;REQUEST
  (def eCommSale-request

    {:classifier {:match "request-sale-charge"
                  :fragments ["request" [:transaction :type] [:transaction :class]]}

     :schema-constraints {:on-self ['novate.rt-constraints.pan/pan-XOR-token  ;; pan OR token - NOT-both
                                    'novate.rt-constraints.pan/pan-AND-security-code]}   ;;pan AND security-code

     ;; actual templates to use for the guts of the schema	(required)
     :templates {:transaction {[[:transaction :class] :mandatory] {:value "charge"} ;; no need for :type when :value(s) is provided (we can infer it)
                               [[:transaction :type] :mandatory] {:value "sale"}
                               [[:transaction :amount :value] :mandatory] {:type "java.lang.String"} ;; we can go from the String to the Class very easily - `Class.forName();`
                               [[:transaction :amount :currency] :mandatory] {:type "java.lang.String"
                                                                              :constraints {:valid :currency}}
                               [[:transaction :transactionDateTime] :mandatory] {:type "java.lang.String"
                                                                                 :constraints {:format :RFC-3339}}
                               [[:transaction :referenceNumber] :mandatory] {:type "java.lang.String"}}

                 :cardholder {[[:cardholder :firstName] :optional] {:type "java.lang.String"
                                                                    :constraints {:max-size 15}}
                              [[:cardholder :lastName] :optional] {:type "java.lang.String"
                                                                   :constraints {:max-size 20}}
                              [[:cardholder :postCode] :optional] {:type "java.lang.String"}
                              [[:cardholder :streetAddress] :optional] {:type "java.lang.String"}}

                 :card {[[:card :expiryDate] :mandatory] {:type "java.lang.String"
                                                          :constraints {:size 4
                                                                        :format #"\d+"}}
                        [[:card :securityCode] :conditional] {:type "java.lang.String"
                                                              :constraints {:size 3
                                                                            :format #"\d+"}}
                        [[:card :pan] :conditional] {:type "java.lang.String"
                                                     :constraints {:max-size 16
                                                                   :format #"\d+"}}}

                 :terminal {[[:terminal :referenceUuid] :mandatory] {:type "String"
                                                                     :constraints {:format :uuid}}}

                 :ecommerce {[[:ecommerce :security] :mandatory] {:value "ssl"}}

                 :token {[[:token :token] :conditional] {:type "java.lang.String"
                                                         :constraints {:size 6
                                                                       :format :alphanum}}}
  ...
  (def eCommSale-response
    {:id {:expected "response-sale-charge"
          :actual ["response" [:transaction :type] [:transaction :class]]}
     :schema-constraints {:on-other ["novate.rt-constraints.pan/request_pan-AND-response_avsCodeResult"]}
     :templates {
  ...
  (def eCommSale-req+resp-validation-config
    {:request eCommSale-request
     :response eCommSale-response})


  ;; sample usage of request+response validation
  (with-dependent-validation ...request... eCommSale-request eCommSale-response
                             ...some-code-that-produces-a-response...)
